			...
			function loadRegionData(region){
				...
				map.load(url, {}, function(){
					...
					$(this).children('a.bullet').each(function(){
						...
						$("#" + $(this).attr('id') + "-box").css({left: addpx(Number(coords[0]) + rempx(settings.bulletWidthOffset)), top: addpx(Number(coords[1]) + rempx(settings.bulletHeightOffset))});
						$(this).css({left: addpx(Number(coords[0]) - rempx(settings.bulletWidthOffset)), top: addpx(Number(coords[1]) - rempx(settings.bulletHeightOffset))})
							   .hide()
							   .hover(function(){showPopup($(this).attr('id'));}, function(){hidePopup($(this).attr('id'));})
							   .fadeIn('fast');							
					});
					...
					// bloc permettant de resize les textes a la largeur annoncée de la div
/*					$(this).children('div.inert-text').each(function(){
